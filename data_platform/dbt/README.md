...
The 31 Stage 1 models implement `model-contracts.yaml`; schema and singular tests enforce their keys, relationships, ranges and revenue reconciliation. Sources are source-shaped raw tables created by `data_platform.ingestion` from the contract in `../raw_schema.sql`.
